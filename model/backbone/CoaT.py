...
"""
MindSpore implementation of `CoaT`.
Refer to Co-Scale Conv-attentional Image Transformer
"""
...
class ConvRelPosEnc(nn.Cell):
    ...
    def __init__(self, Ch, h, window):
        ...
        Initialization.
            ...
            window: Window size(s) in convolutional relative positional encoding. It can have two forms:
                    1. An integer of window size, which assigns all attention heads with the same window
        ...
        self.conv_list = nn.SequentialCell()
        self.head_splits = []
    ...
    def construct(self, q, v, size):
        ...
        v_img_shape = v_img.shape
        v_img = v_img.reshape(v_img_shape[0], v_img_shape[1], H, W, v_img_shape[-1])\
            .transpose(0, 1, 4, 2, 3).reshape(v_img_shape[0], -1, H, W)  # Shape: [B, h, H*W, Ch] -> [B, h*Ch, H, W].
        # v_img = rearrange(v_img, 'B h (H W) Ch -> B (h Ch) H W', H=H, W=W)
        v_img_list = ms.ops.split(v_img, self.channel_splits, axis=1)  # Split according to channels.
        conv_v_img_list = [conv(x) for conv, x in zip(self.conv_list, v_img_list)]
        conv_v_img = ms.ops.cat(conv_v_img_list, axis=1)
        conv_v_img_shape = conv_v_img.shape
        conv_v_img = conv_v_img.reshape(conv_v_img_shape[0], h, conv_v_img_shape[1]//h,
                                        conv_v_img_shape[2], conv_v_img_shape[3])\
            .transpose(0, 1, 3, 4, 2).reshape(conv_v_img_shape[0], h, -1, conv_v_img_shape[1]//h)

        EV_hat_img = q_img * conv_v_img
        zero = ms.ops.zeros((B, h, 1, Ch), dtype=q.dtype)
        EV_hat = ms.ops.cat((zero, EV_hat_img), axis=2)  # Shape: [B, h, N, Ch].
        return EV_hat
...
class FactorAtt_ConvRelPosEnc(nn.Cell):
    ...
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., shared_crpe=None):
        ...
        self.attn_drop = nn.Dropout(p=attn_drop)  # Note: attn_drop is actually not used.
    ...
    def construct(self, x, size):
        ...
        k_softmax = ms.ops.softmax(k, axis=2)
        k_softmax_T_dot_v = ms.ops.einsum('b h n k, b h n v -> b h k v', k_softmax, v)  # Shape: [B, h, Ch, Ch].
        factor_att = ms.ops.einsum('b h n k, b h k v -> b h n v', q, k_softmax_T_dot_v)  # Shape: [B, h, N, Ch].
        # Convolutional relative position encoding.
        crpe = self.crpe(q, v, size=size)  # Shape: [B, h, N, Ch].
        # Merge and reshape.
        x = self.scale * factor_att + crpe
        x = x.transpose(0, 2, 1, 3).reshape(B, N, C)  # Shape: [B, h, N, Ch] -> [B, N, h, Ch] -> [B, N, C].
...
class ConvPosEnc(nn.Cell):
    ...
    def __init__(self, dim, k=3):
        super().__init__()
        self.proj = nn.Conv2d(dim, dim, kernel_size=k, stride=1, pad_mode='pad', padding=k//2, group=dim)
    ...
    def construct(self, x, size):
        ...
        feat = img_tokens.transpose(0, 2, 1).view(B, C, H, W)
        x = self.proj(feat) + feat
        x = x.flatten(start_dim=2).transpose(0, 2, 1)

        # Combine with CLS token.
        x = ms.ops.cat((cls_token, x), axis=1)
...
class ParallelBlock(nn.Cell):
    ...
    def __init__(self, dims, num_heads, mlp_ratios=None, qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm,
    ...
    def interpolate(self, x, output_size, size):
        ...
        img_tokens = ms.ops.interpolate(img_tokens, size=output_size, mode='bilinear')
        img_tokens = img_tokens.reshape(B, C, -1).transpose(0, 2, 1)

        out = ms.ops.cat((cls_token, img_tokens), axis=1)
...
class CoaT(nn.Cell):
    ...
    def __init__(self, patch_size=16, in_chans=3, num_classes=1000, embed_dims=None,
                 serial_depths=None, parallel_depth=0,
                 num_heads=0, mlp_ratios=None, qkv_bias=True, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
                 drop_path_rate=0., norm_layer=partial(nn.LayerNorm, epsilon=1e-6),
                 return_interm_layers=False, out_features=None, crpe_window=None, ):
        super().__init__()
        if embed_dims is None:
            embed_dims = [0, 0, 0, 0]
        if serial_depths is None:
            serial_depths = [0, 0, 0, 0]
        if mlp_ratios is None:
            mlp_ratios = [0, 0, 0, 0]
        if crpe_window is None:
        ...
        # Class tokens.
        self.cls_token1 = ms.Parameter(ms.ops.zeros((1, 1, embed_dims[0])))
        self.cls_token2 = ms.Parameter(ms.ops.zeros((1, 1, embed_dims[1])))
        self.cls_token3 = ms.Parameter(ms.ops.zeros((1, 1, embed_dims[2])))
        self.cls_token4 = ms.Parameter(ms.ops.zeros((1, 1, embed_dims[3])))
        ...
        crpe1 = ConvRelPosEnc(Ch=embed_dims[0] // num_heads, h=num_heads, window=crpe_window)
        crpe2 = ConvRelPosEnc(Ch=embed_dims[1] // num_heads, h=num_heads, window=crpe_window)
        crpe3 = ConvRelPosEnc(Ch=embed_dims[2] // num_heads, h=num_heads, window=crpe_window)
        crpe4 = ConvRelPosEnc(Ch=embed_dims[3] // num_heads, h=num_heads, window=crpe_window)

        # Enable stochastic depth.
        dpr = drop_path_rate

        # Serial blocks 1.
        self.serial_blocks1 = nn.SequentialCell([
            SerialBlock(
        ...
        if self.parallel_depth > 0:
            self.parallel_blocks = nn.SequentialCell([
                ParallelBlock(
                    ...
                    shared_crpes=[crpe1, crpe2, crpe3, crpe4]
                )
        ...
        # Initialize weights.
        self.cls_token1 = initializer(TruncatedNormal(sigma=.02), self.cls_token1.shape, self.cls_token1.dtype)
        self.cls_token2 = initializer(TruncatedNormal(sigma=.02), self.cls_token2.shape, self.cls_token2.dtype)
        self.cls_token3 = initializer(TruncatedNormal(sigma=.02), self.cls_token3.shape, self.cls_token3.dtype)
        self.cls_token4 = initializer(TruncatedNormal(sigma=.02), self.cls_token4.shape, self.cls_token4.dtype)
    ...
    def construct_features(self, x0):
        """ Construct Features """
        B = x0.shape[0]
        ...
        x1_nocls = x1_nocls.reshape(B, H1, W1, -1).permute(0, 3, 1, 2)
        ...
        x2_nocls = x2_nocls.reshape(B, H2, W2, -1).permute(0, 3, 1, 2)
        ...
        x3_nocls = x3_nocls.reshape(B, H3, W3, -1).permute(0, 3, 1, 2)
        ...
        x4_nocls = x4_nocls.reshape(B, H4, W4, -1).permute(0, 3, 1, 2)
        ...
        x3_cls = x3[:, :1]
        x4_cls = x4[:, :1]
        merged_cls = ms.ops.cat((x2_cls, x3_cls, x4_cls), axis=1)  # Shape: [B, 3, C].
        merged_cls = self.aggregate(merged_cls).squeeze(axis=1)  # Shape: [B, C].
        return merged_cls

    def construct(self, x):
        if self.return_interm_layers:  # Return intermediate features (for down-stream tasks).
            return self.construct_features(x)
...
if __name__ == '__main__':
    ...
    coat = CoaT(patch_size=4, embed_dims=[152, 152, 152, 152], serial_depths=[2, 2, 2, 2], parallel_depth=6,
                num_heads=8, mlp_ratios=[4, 4, 4, 4])
    output = coat(dummy_input)
    print(output.shape)
